...
function addDebug(msg) {
  // Guardamos últimos 8 mensajes para tener más contexto
  debugMessages.value = [msg, ...debugMessages.value].slice(0, 8)
}
// --------------------
...
let rawSpyUnbind = null // Para limpiar el espía

function wireWsSyncAndSubs() {
  const ws = getCurrentWebSocket()
  if (!ws) return

  // === ESPÍA DE SOCKET NATIVO (BYPASS @lib/ws) ===
  const rawSpy = (event) => {
    try {
      const txt = event.data
      // Ignoramos pongs y mensajes vacíos para no saturar
      if (txt.includes('pong')) return

      // Intentar parsear solo para ver el tipo
      const parsed = JSON.parse(txt)
      const type = parsed.type || 'unknown'

      // Logueamos DIRECTO lo que llega
      addDebug(`RAW IN: ${type} (len=${txt.length})`)

      // Si es sensor_update, forzamos actualización manual por si addWsListener falla
      ...
      addDebug(`RAW IN (No-JSON): ${event.data.substring(0, 20)}...`)
    }
  }

  // Agregar listener nativo
...
function attachBusListener() {
  offBus = addWsListener((parsed) => {
    ...
    if (updates.length > 0) {
      addDebug(`LIB RX: ${updates.length} updates`)
    }
...
<template>
  <div>
    <div class="debug-panel">
      ...
      <hr />
      <small>Live Status Keys: {{ Object.keys(liveSensorStatus) }}</small>
    </div>
